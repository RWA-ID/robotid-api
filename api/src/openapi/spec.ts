    ...
    schemas: {
      ...
      KeyInfo: {
        ...
        properties: {
          ...
          caps: { type: 'object', properties: { units: { type: 'integer', nullable: true }, namespaces: { type: 'integer', nullable: true } } },
          usage: { type: 'object', properties: { requestCount: { type: 'integer' }, unitCount: { type: 'integer' }, namespaceCount: { type: 'integer' } } },
  ...
  paths: {
    '/health': {
      get: {
        ...
        responses: { '200': { description: 'Service status', content: { 'application/json': { example: { status: 'ok', chainId: 1, parent: 'robot-id.eth', store: { backend: 'redis', ok: true }, ts: 1780466612411 } } } } },
      },
    },
    '/auth/tiers': {
      get: {
    ...
    '/auth/namespace/reserve': {
      post: {
        ...
        description: [
          ...
          'No active subscription is required to reserve a first (primary) slug (checkout happens before',
          'the tx confirms); the reservation is consumed by the on-chain `Subscribed` watcher, which',
          'provisions `<slug>.robot-id.eth` to the wallet on payment. Reservations are additive up to the',
          "tier's namespace cap (Manufacturer 1, OEM 5, Enterprise unlimited); a single-namespace wallet",
          're-pointing to a new slug releases its previous one. Use /auth/namespace/release to free a slug.',
        ].join(' '),
        ...
        responses: {
          '200': { description: 'Reserved', content: { 'application/json': { example: { reserved: true, slug: 'boston-dynamics', name: 'boston-dynamics.robot-id.eth', subscriber: '0x…', namespaces: ['boston-dynamics'], cap: 5 } } } },
          '400': { description: 'Invalid signature or slug', content: { 'application/json': { schema: { $ref: '#/components/schemas/Error' } } } },
          '401': { $ref: '#/components/responses/Unauthorized' },
          '403': { description: 'Namespace cap reached for tier', content: { 'application/json': { schema: { $ref: '#/components/schemas/Error' } } } },
          '409': { description: 'Slug already reserved by another wallet', content: { 'application/json': { schema: { $ref: '#/components/schemas/Error' } } } },
        },
      },
    },
    '/auth/namespace/release': {
      post: {
        tags: ['Auth'], summary: 'Release an OEM namespace reservation (SIWE)',
        description: [
          'Frees a slug the wallet currently holds so a multi-namespace subscriber can swap brands or give',
          'one up. Only affects the off-chain reservation; any name already provisioned on-chain is unaffected.',
        ].join(' '),
        requestBody: { required: true, content: { 'application/json': { schema: { type: 'object', required: ['address', 'message', 'signature', 'slug'], properties: { address: Address, message: { type: 'string', example: 'Release boston-dynamics.robot-id.eth for 0x… on robot-id.eth' }, signature: { type: 'string', example: '0x…' }, slug: { type: 'string', example: 'boston-dynamics' } } } } } },
        responses: {
          '200': { description: 'Released', content: { 'application/json': { example: { released: true, slug: 'boston-dynamics', namespaces: [] } } } },
          '400': { description: 'Invalid signature or slug', content: { 'application/json': { schema: { $ref: '#/components/schemas/Error' } } } },
          '401': { $ref: '#/components/responses/Unauthorized' },
          '404': { description: 'Slug not held by this wallet', content: { 'application/json': { schema: { $ref: '#/components/schemas/Error' } } } },
        },
      },
    },
    ...
    '/api/v1/robots': {
      post: {
        ...
        responses: {
          ...
          '401': { $ref: '#/components/responses/Unauthorized' },
          '402': { $ref: '#/components/responses/PaymentRequired' },
          '403': { description: 'Unit cap reached for tier', content: { 'application/json': { schema: { $ref: '#/components/schemas/Error' } } } },
        },
      },
    },
    '/api/v1/robots/batch/preauthorize': {
      post: {
        ...
        responses: {
          ...
          '401': { $ref: '#/components/responses/Unauthorized' },
          '402': { $ref: '#/components/responses/PaymentRequired' },
          '403': { description: 'Batch exceeds the tier unit cap', content: { 'application/json': { schema: { $ref: '#/components/schemas/Error' } } } },
